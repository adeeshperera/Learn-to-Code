...
// Updates an existing guitar’s details in a database
export async function saveGuitar(id, make, model) {
	const guitar = await getById(id);

	if (guitar) {
		guitar.make = make; // Updates the make field of the guitar with the new make value
		guitar.model = model;
		guitar.make_lower = make.toLowerCase();

		guitar.save(); // Saves the updated guitar document back to the database
	}
}
